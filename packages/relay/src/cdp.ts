 ...
 *
 * Supports tab-based targeting via an optional targetId.
 * All sessions can share one Chrome process on one cdpPort.
 */
...
export async function listTabs(cdpPort: number): Promise<CDPTarget[]> {
...
export class CDPSession {
  ...
  async connect(screenshotCallback: (jpeg: Buffer) => void): Promise<void> {
    ...
    const targets = await listTabs(this.cdpPort);

    let page: CDPTarget | undefined;
    if (this.targetId) {
      page = targets.find(t => t.targetId === this.targetId);
      if (!page) throw new Error(`[cdp:${this.cdpPort}] No tab found with targetId=${this.targetId}`);
    } else {
    ...
    console.log(`[cdp:${this.cdpPort}] connected → ${page.url}${this.targetId ? ` (targetId=${this.targetId})` : ''} (viewport ${this.vpW}x${this.vpH})`);
  }
