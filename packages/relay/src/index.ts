...
import { CDPSession, listTabs } from './cdp.js';
...
// ---------------------------------------------------------------------------
// Port / target-id parsing
// ---------------------------------------------------------------------------
function parseList(val: string | undefined, fallback: string): string[] {
  return (val ?? fallback).split(',').map(s => s.trim()).filter(Boolean);
}

const ports     = parseList(process.env.PORTS ?? process.env.PORT, '7001').map(Number);
const cdpPort   = Number(process.env.CDP_PORT ?? 9222);
const targetIds = process.env.TARGET_IDS?.split(',').map(s => s.trim()).filter(Boolean);

if (targetIds && targetIds.length !== ports.length) {
  console.error(`[relay] TARGET_IDS has ${targetIds.length} entries but PORTS has ${ports.length}. Must match.`);
  process.exit(1);
}

// ---------------------------------------------------------------------------
// One RelaySession per port
// ---------------------------------------------------------------------------
...
class RelaySession {
  ...
  constructor(private port: number, cdpPort: number, targetId?: string) {
    this.cdp = new CDPSession(cdpPort, targetId);
  ...
  start(): void {
    ...
    // List available Chrome tabs — so user can discover targetIds
    app.get('/tabs', async (_req, res) => {
      try {
        const tabs = await listTabs(this.cdp.cdpPort);
        res.json(tabs.map(t => ({ targetId: t.targetId, url: t.url, title: t.title })));
      } catch {
        res.status(502).json({ error: 'Chrome not reachable' });
      }
    });
    ...
    wss.on('connection', (ws, req) => {
      ...
      if (url === '/stream/push') {
        console.log(`[relay:${this.port}] ▲ streamer connected (extension frame push — CDP mode ignores this)`);
        ws.on('message', () => {});
    ...
    server.listen(this.port, async () => {
      ...
      console.log(`  Tabs         http://localhost:${this.port}/tabs\n`);
      ...
      try {
        ...
        console.error(`[relay:${this.port}] Start Chrome with --remote-debugging-port=${this.cdp.cdpPort} then restart relay\n`);
        this.scheduleReconnect();
      }
    });
  }
...
for (let i = 0; i < ports.length; i++) {
  new RelaySession(ports[i]!, cdpPort, targetIds?.[i]).start();
}
